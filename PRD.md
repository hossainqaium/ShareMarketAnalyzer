...
| F46 | Trend-Break detection (Spike tab): `detect_regime_break()` fits log-price OLS over the longest of {252,189,126,90,63} sessions (excluding the last 5 as a holdout) that qualifies as a clean downtrend/uptrend (R² ≥ 0.5, ≥0.08%/day drift) or tight range (≤16% band width); the holdout sessions are z-scored against the fitted trend/range and a break is flagged at |z| ≥ 2.2. Scored 0–100 (40% break magnitude, 30% volume confirmation, 30% MACD/candle/divergence agreement) and merged into the Spike list as `kind: "trend-break"` alongside same-day % spikes, filterable by kind; excluded for non-equity/illiquid instruments. |
